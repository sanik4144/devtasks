...
const ListTasks = () => {
  ...
  return (
    <div className={`min-h-screen p-6 font-sans antialiased transition-colors duration-300 ${dark ? 'bg-zinc-950' : 'bg-[#FDFDFD]'}`}>
      ...
      <div
        className={`max-w-2xl mx-auto rounded-4xl shadow-lg p-8 border transition-colors duration-300 ${dark ? 'bg-zinc-900 border-zinc-700' : 'bg-white border-neutral-100'}`}
      >
                  ...
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    {editingId === task.id ? (
                      ...
                      <div className="flex items-center gap-3">
                        <span
                          className={`font-semibold text-lg ${task.completed ? 'line-through text-neutral-400' : dark ? 'text-white' : 'text-black'}`}
                        >
                          {task.text}
                        </span>
                        <div className="relative">
                          ...
                          {activeDropdownId === task.id && (
                            <div
                              className={`absolute top-7 left-0 z-10 rounded-xl shadow-lg border p-2 flex flex-col gap-1 min-w-[120px] ${dark ? 'bg-zinc-800 border-zinc-700' : 'bg-white border-neutral-200'}`}
                            >
                              {categories.map((cat) => (
                                <button
                                  key={cat}
