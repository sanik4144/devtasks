...
const ListTasks = () => {
  ...
  const filteredTasks = tasks.filter((task) => {
    const matchFilter = filter === 'ACTIVE' ? !task.completed : filter === 'COMPLETED' ? task.completed : true
  ...
  const saveEdit = (id) => {
    ...
    const updatedTasks = tasks.map((task) => (task.id === id ? { ...task, text: trimmed } : task))
 ...
 toast(`${completedTasks.length} completed ${completedTasks.length === 1 ? 'task' : 'tasks'} moved`, {
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (activeDropdownId && dropdownRef.current && !dropdownRef.current.contains(e.target)) {
  ...
  const restoreDeletedTask = (taskToRestore) => {
    ...
    if (deletedTasks) {
      deletedTasks = JSON.parse(deletedTasks).filter((t) => t.id !== taskToRestore.id)
  ...
  const updateCategory = (taskId, newCategory) => {
    const updatedTasks = tasks.map((t) => (t.id === taskId ? { ...t, category: newCategory } : t))
    setTasks(updatedTasks)
    localStorage.setItem('tasks', JSON.stringify(updatedTasks))
    setActiveDropdownId(null)
    ...
    <div className={`min-h-screen p-6 font-sans antialiased transition-colors duration-300 ${dark ? 'bg-zinc-950' : 'bg-[#FDFDFD]'}`}>
      ...
      >
        <div className="flex justify-between items-center mb-8">
          <h1 className={`text-3xl font-black uppercase ${dark ? 'text-white' : 'text-black'}`}>Task List</h1>
          <ThemeToggle />
        </div>
                      ...
                      <div className="flex items-center gap-3">
                        <span
                          ...
                          className={`font-semibold text-lg cursor-text transition-opacity duration-200 hover:opacity-80 ${
                            task.completed ? 'line-through text-neutral-400' : dark ? 'text-white' : 'text-black'
                          }`}
                        >
                          {task.text}
                        </span>

                        <div className="relative" ref={activeDropdownId === task.id ? dropdownRef : null}>
                          <button
                            type="button"
                          ...
                          {activeDropdownId === task.id && (
                            <div
                              className={`absolute top-7 left-0 z-10 rounded-xl shadow-lg border p-2 flex flex-col gap-1 min-w-[120px] transition-all duration-200 ${
                                dark ? 'bg-zinc-800 border-zinc-700' : 'bg-white border-neutral-200'
                              }`}
                            >
                              {categories.map((cat) => (
                                <button
                ...
                <div className="flex items-center gap-2 ml-3 shrink-0">
                  {!task.completed && editingId !== task.id && (
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl border font-bold text-sm transition-all duration-300 ${
                        dark ? 'border-white text-white hover:bg-white hover:text-black' : 'border-black text-black hover:bg-black hover:text-white'
                      }`}
                    >
                      Edit
                    </button>
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl transition-all duration-300 font-bold text-sm ${
                      dark ? 'bg-white text-black hover:bg-gray-100' : 'bg-black text-white hover:bg-neutral-800'
                    }`}
                  >
                    Delete
                  </button>
