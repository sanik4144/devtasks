...
const ListSnippets = () => {
  ...
  return (
    <div
      className={`min-h-screen overflow-y-auto p-4 sm:p-6 font-sans antialiased transition-colors duration-300 ${dark ? "bg-zinc-950" : "bg-[#FDFDFD]"
        }`}
    >
      <div
      ...
      >
        {/* Header */} <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8">
          <h1
            className={`text-2xl sm:text-3xl font-black uppercase ${dark ? "text-white" : "text-black"
              }`}
          >
            Snippet Registry </h1>
        ...
        <input
          ...
          className={`w-full mb-4 px-4 py-3 rounded-2xl border-2 outline-none font-black uppercase tracking-widest text-sm transition-all duration-200 ${dark
        ...
        <div className="flex justify-center mb-6">
          <div
            className={`flex flex-wrap justify-center gap-2 p-1 border rounded-3xl ${dark
          ...
          >
            {categories.map((cat) => (
              <button
                ...
                className={`px-5 py-2 rounded-full text-xs font-black uppercase tracking-widest transition-all duration-200 cursor-pointer ${selectedCategory === cat
                    ? dark
                      ? "bg-white text-black"
                      : "bg-black text-white"
                    : dark
            ...
            {filteredSnippets.map((sn) => (
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <h3
                    className={`font-semibold text-base sm:text-lg ${dark ? "text-white" : "text-black"
                      }`}
                  >
                    {sn.title}
                  </h3>
                  ...
                  <span
                    className={`text-[11px] font-black uppercase px-2 py-1 rounded-full ${dark
                ...
                {/* Code Block */}
                <pre
                  className={`overflow-x-auto rounded-xl p-4 font-mono text-xs ${dark
                ...
                <div className="flex flex-col sm:flex-row gap-2 sm:justify-end">
                  ...
                  <Link
                    ...
                    className={`px-4 py-2 rounded-xl border font-bold text-sm text-center transition-all duration-300 active:scale-95 ${dark
                        ? "border-white text-white hover:bg-white hover:text-black"
                        : "border-black text-black hover:bg-black hover:text-white"
                      }`}
                  >
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl transition-all duration-300 font-bold text-sm active:scale-95 ${dark
        ...
        <div className="mt-12 border-t border-neutral-100 dark:border-zinc-800 pt-6">
          <Link
            to="/snippetvault"
            className={`inline-flex items-center gap-2 text-xs sm:text-sm font-black uppercase tracking-widest transition-all duration-300 ${dark
